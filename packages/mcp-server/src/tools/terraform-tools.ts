...
export function registerTerraformTools(server: McpServer, bridge: BridgeClient): void {
  server.tool(
    ...
    async () => {
      try {
        const result = await bridge.request('mcp_generate_hcl') as Record<string, string>;
  ...
  server.tool(
    ...
    async () => {
      try {
        const result = await bridge.request('mcp_get_hcl_files') as Record<string, string>;
        const files = Object.entries(result);
        if (files.length === 0) {
          return {
  ...
  server.tool(
    ...
    async (params) => {
      ...
      try {
        const result = await bridge.request('mcp_run_terraform', { command: cmd });
        return {
  ...
  server.tool(
    ...
    async () => {
      try {
        const result = await bridge.request('mcp_get_deployment_status');
        return {
          content: [{ type: 'text' as const, text: JSON.stringify(result, null, 2) }],
        };
      } catch (err: any) {
